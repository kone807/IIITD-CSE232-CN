...
class Node {
 ...
 public:
  ...
  void addInterface(string ip, string connip, Node *nextHop, int cost=1) {
    NetInterface eth;
    eth.setip(ip);
  ...
  // removes specified interface
  void removeInterface(string ip, string connip)
  {
    for(int i=0; i<interfaces.size(); i++)
    {
  ...
  // added update table entry function
  void updateTblEntry(string myip, string dstip, int cost)
  {
    for(int i=0; i<mytbl.tbl.size(); i++)
    {
      RoutingEntry currEntry = mytbl.tbl[i];

      if(currEntry.ip_interface==myip && currEntry.dstip==dstip)
      {
        //cout<<"DONE"<<endl;
        currEntry.cost = cost;
        mytbl.tbl[i] = currEntry;
      }
    }
  }
  ...
  void sendMsg(){
    ...
    for (int i = 0; i < interfaces.size(); ++i) {
      ...
      // do not send if they are not connected
      bool send=true;
      for(RoutingEntry e : mytbl.tbl)
      {
        if(e.dstip==msg.recvip && e.ip_interface==msg.from && e.cost>=16)
        {
          send=false;
          break;
        }
