...
void routingAlgo(vector<RoutingNode*> nd){
  ...
  // run RIP to check convergence
   ...
   /* 
    We observe that the convergence is very slow. This is the count to infinity problem
    where infinity = 16.
   */
}
  ...
  while(i<m)
  {
    ...
    for(int j=0; j<n; j++)
    {
      ...
      if(selfTable[j].dstip == recvTable[i].dstip)
      {
        ...
        // check if a lower cost path is available or connection is changed
        bool b1 = selfTable[j].cost > recvTable[i].cost+c;
        bool b2 = selfTable[j].nexthop == msg->from;
        
        // update costs if either b1 or b2 is true and connection still exists
         if((b1 || b2) && recvTable[i].cost<=16)
         {
